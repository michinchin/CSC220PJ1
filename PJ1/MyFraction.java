 ...
 *
 * Warning: Don't copy projects from previous semesters and others!
 *
 * /*************************************************************************************
 *
 ...
 *    Please Refer to strategy in given GCD recursive method.
 *
...
public class MyFraction implements MyFractionInterface, Comparable<MyFraction> {
    ...
    public MyFractionInterface add(MyFractionInterface secondFraction) {
        MyFraction frac = new MyFraction((this.num * ((MyFraction) secondFraction).den) + ((((MyFraction) secondFraction).num) * this.den),
                                         (this.den * (((MyFraction) secondFraction).den)));
        frac.reduceMyFractionToLowestTerms();
        // implement this method!
        // a/b + c/d is (ad + cb)/(bd)
    ...
    public MyFractionInterface multiply(MyFractionInterface secondFraction) {
        ...
        MyFraction frac = new MyFraction((this.num * (((MyFraction) secondFraction).num)), (((MyFraction) secondFraction).den) * this.den);
        frac.reduceMyFractionToLowestTerms();
        // implement this method!
        // a/b * c/d is (ac)/(bd)
        // return result which is a new MyFraction object
    ...
    public MyFractionInterface divide(MyFractionInterface secondFraction) {
        MyFraction frac = new MyFraction(this.num * (((MyFraction) secondFraction).den), this.den * ((MyFraction) secondFraction).num);
        // implement this method!
        // a/b / c/d is (ad)/(bc)
        if ((((MyFraction) secondFraction).den) == 0) {
            throw new MyFractionException();
        }
    ...
    public boolean equals(Object other) {
        if (other != null) {
            MyFraction others = (MyFraction) other;
            others.reduceMyFractionToLowestTerms();
            return (this.num == others.num && this.den == others.den);
        }
        // implement this method!
    ...
    private void reduceMyFractionToLowestTerms() {
        ...
        num = num / gcd;
        den = den / gcd;
        //        num = (num / (GCD(Math.abs(num), Math.abs(den))));
        //        den = (den / (GCD(Math.abs(num), Math.abs(den))));
